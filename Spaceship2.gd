...
func _on_Spaceship2_body_entered(body):
	print (body);
	if (body.get_name() == "Goal"):
		$Spaceship_sprite.play("land");
		var goal_pos= Vector2();
		var ship_pos = Vector2();
		goal_pos = get_node("/root/Level2/Moon/Goal").position;
		...
		get_node("/root/Level2/HUD/PopupDialog").popup_centered();
		cleared = true;
	elif (body.get_name() == "Crash" && cleared == false):
		get_node("/root/Level2/HUD/TryAgain").popup_centered();
